...
**执行命令模板**：
```bash
# 调用 follow-builders skill（假设 skills 目录结构为 flat layout）
python ../follow-builders/scripts/run.py
# 如果 follow-builders 安装在其他位置，请调整路径
```
...
**执行命令模板**：
```bash
# 调用 video-content-collector skill（假设 skills 目录结构为 flat layout）
python ../video-content-collector/scripts/collect.py --topic "选题主题" --days 7
# 如果 video-content-collector 安装在其他位置，请调整路径
```
...
本 skill 支持在 WorkBuddy、Claude Code、OpenClaw、Codex 中使用。各平台路径约定不同，请根据你的环境调整 `scripts/` 目录的引用方式。

### WorkBuddy（推荐）

```bash
# skills 目录结构（flat layout）
~/.workbuddy/skills/
├── ai-frontier-video-pipeline/
│   ├── SKILL.md
│   └── scripts/
├── follow-builders/
└── video-content-collector/

# 调用其他 skill（相对路径）
python ../follow-builders/scripts/run.py
python ../video-content-collector/scripts/collect.py --topic "主题"

# 调用本 skill 脚本（相对路径）
python scripts/slide_template.py --config output/script.json --ep 01 --output-dir slides/
python scripts/cover_template.py --title "标题" --ep 01 --output-dir covers/
python scripts/render_video.py --slides-dir slides/ --voice output/voice.mp3 --ep 01 --output output/ep01.mp4
```
...
# 或者手动运行脚本（请调整 SKILLS_DIR 变量）
export SKILLS_DIR=~/.claude/skills

python $SKILLS_DIR/follow-builders/scripts/run.py
python $SKILLS_DIR/ai-frontier-video-pipeline/scripts/slide_template.py \
  --config output/script.json --ep 01 --output-dir slides/
python $SKILLS_DIR/ai-frontier-video-pipeline/scripts/render_video.py \
  --slides-dir slides/ --voice output/voice.mp3 --ep 01 --output output/ep01.mp4
```
...
python $SKILLS_DIR/follow-builders/scripts/run.py
python $SKILLS_DIR/ai-frontier-video-pipeline/scripts/cover_template.py \
  --title "标题" --ep 01 --output-dir covers/
python $SKILLS_DIR/ai-frontier-video-pipeline/scripts/render_video.py \
  --slides-dir slides/ --voice output/voice.mp3 --ep 01 --output output/ep01.mp4
```
...
# 或者手动运行脚本（请调整 SKILLS_DIR 变量）
export SKILLS_DIR=~/.codex/skills
...
| 平台 | 默认 skills 目录 | 环境变量示例 |
|--------|---------------------|----------------------|
| WorkBuddy | `~/.workbuddy/skills/` | 使用相对路径 `../` 即可 |
| Claude Code | `~/.claude/skills/` | `SKILLS_DIR=~/.claude/skills` |
| OpenClaw | `~/.openclaw/skills/` | `SKILLS_DIR=~/.openclaw/skills` |
| Codex | `~/.codex/skills/` | `SKILLS_DIR=~/.codex/skills` |

> **提示**：如果你把 skills 安装在了非默认位置，请将上述所有路径替换为你自己的实际路径。
